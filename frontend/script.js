...
// Backend deployed on Prisma Compute (project "mess-feedback", ap-southeast-1).
// Keep the /api suffix and no trailing slash.
const PRODUCTION_API_URL = 'https://v426yu4750tx6k1ume80t3va.sin.prisma.build/api';

const isLocalhost = ['localhost', '127.0.0.1', ''].includes(location.hostname);

// API base URL - Make sure this matches your server
const API_URL = isLocalhost ? 'http://localhost:8080/api' : PRODUCTION_API_URL;
// Backup URL in case the primary doesn't work
const BACKUP_API_URL = isLocalhost ? 'http://127.0.0.1:8080/api' : PRODUCTION_API_URL;

// Authentication state
let currentUser = null;
let userType = null;

// The API is stateless: logging in returns a signed token that must be sent on
// every authenticated request. Server sessions no longer exist, so this token
// is what keeps the user logged in across page loads.
...
async function logout() {
    try {
        ...
        // Fall through: the token is discarded either way.
    } finally {
